...
class MittagLefflerPlotter:
    ...
    Function 1: f1(t) = E_{2*alpha, 1}(-(\omega*t)^{2*alpha})
    Function 2: f2(t) = (\omega*t)^\alpha E_{2*alpha, 1+\alpha}(-(\omega*t)^{2*alpha})
    New function 3: g(t) = t E_{2*alpha, 2}(-(\omega*t)^{2*alpha})
    """
    ...
    def _approx_g(self, t, alpha):
        """
        New function g(t) = tE_{self.m*alpha, 2}(-(\omega*t)^{self.m*alpha})
        g(t) = sum_{k=0}^{\infty} [ (-1)^k * t * (\omega*t)^{self.m*alpha*k} / Gamma(self.m*alpha*k + 2) ]
        """
        result = np.zeros_like(t, dtype=float)

        for k in range(self.K):
            # Numerator: (-1)^k * t * (omega*t)^{2*alpha*k}
            exponent = float(self.m) * alpha * k
            numerator = ((-1.0) ** k) * t * np.power(self.omega * t, exponent)

            # Denominator: Gamma(2*alpha*k + 2)
            gamma_arg = float(self.m) * alpha * k + 2
            denominator = gamma(gamma_arg)
    ...
    def calculate_f1(self, alpha):
        if alpha == 0.5:
            ...
            label = r'$E_{2\alpha, 1}(-z)$: $\alpha=0.5$ ($e^{-\omega t}$)'
        elif alpha == 1.0:
            # alpha=1.0 -> cos(\omega t)
            y = np.cos(self.omega * self.t)
            label = r'$E_{2\alpha, 1}(-z)$: $\alpha=1.0$ ($\cos(\omega t)$)'
        else:
            y = self._approx_f1(self.t, alpha)
            label = fr'$E_{{2\alpha, 1}}(-z)$: $\alpha={alpha:.2f}$ (近似)'
        return y, label
    ...
    def calculate_f2(self, alpha):
        if alpha == 1.0:
            ...
            label = r'$(\omega t)^\alpha E_{2\alpha, 1+\alpha}(-z)$: $\alpha=1.0$ ($\sin(\omega t)$)'
        else:
            y = self._approx_f2(self.t, alpha)
            label = fr'$(\omega t)^\alpha E_{{2\alpha, 1+\alpha}}(-z)$: $\alpha={alpha:.2f}$'
        return y, label

    def calculate_g(self, alpha):
        if alpha == 1.0:
            # alpha=1.0 -> (1/omega) sin(\omega t)
            y = (1.0 / self.omega) * np.sin(self.omega * self.t)
            label = r'$t E_{2\alpha, 2}(-z)$: $\alpha=1.0$ ($\frac{1}{\omega}\sin(\omega t)$)'
        else:
            y = self._approx_g(self.t, alpha)
            label = fr'$t E_{{2\alpha, 2}}(-z)$: $\alpha={alpha:.2f}$'
        return y, label
    ...
    def plot_function(self, function_name, alphas, filename='plot.png'):
        ...
        if function_name == 'f1':
            ...
            title = r'$f_1(t) = E_{2\alpha, 1}(-(\omega t)^{2\alpha})$ with $\omega=1$'
        elif function_name == 'f2':
            calculator = self.calculate_f2
            title = r'$f_2(t) = (\omega t)^\alpha E_{2\alpha, 1+\alpha}(-(\omega t)^{2\alpha})$ with $\omega=1$'
        elif function_name == 'g':
            calculator = self.calculate_g
            title = r'$g(t) = t E_{2\alpha, 2}(-(\omega t)^{2\alpha})$ with $\omega=1$'
        else:
...
if __name__ == '__main__':
    plotter = MittagLefflerPlotter(t_end=10.0, omega=1.0) # omega=1.0で統一
    ...
    plotter.plot_function(
        ...
        filename='MittagLeffler_F1_v2.png'
    )
    ...
    plotter.plot_function(
        function_name='f2',
        alphas=alphas_to_plot,
        filename='MittagLeffler_F2_v2.png'
    )

    # 3. Plot the graph of the NEW function (t E_{2*alpha, 2})
    print("\n--- Plot of NEW Function g(t) ---")
    plotter.plot_function(
        function_name='g',
        alphas=alphas_to_plot,
        filename='MittagLeffler_G_v2.png'
    )